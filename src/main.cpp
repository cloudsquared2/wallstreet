...
#if defined(NDEBUG)
# error "Blockwallstreetcoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x6870a7eb25c6b6f5732adddfcc9c4a6eebb38f459d47aae8fec66a653cfb7556");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Blockwallstreetcoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Blockwallstreetcoin Signed Message:\n";
...
{
    // Blockwallstreetcoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
static const int64 nTargetTimespan = 10 * 30; // Blockwallstreetcoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Blockwallstreetcoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Blockwallstreetcoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Blockwallstreetcoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x6870a7eb25c6b6f5732adddfcc9c4a6eebb38f459d47aae8fec66a653cfb7556");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "This is blockwallstreetcoin";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xd2a491a1800dfdf4c92e28a48abe888e874225d71c17246d83acb351ecabb5aa"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Blockwallstreetcoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    //// debug print
    printf("BlockwallstreetcoinMiner:\n");
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BlockwallstreetcoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BlockwallstreetcoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BlockwallstreetcoinMiner(CWallet *pwallet)
{
    printf("BlockwallstreetcoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("blockwallstreetcoin-miner");
    ...
    try { loop {
        ...
        printf("Running BlockwallstreetcoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BlockwallstreetcoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BlockwallstreetcoinMiner, pwallet));
}
